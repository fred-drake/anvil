...
- [ ] **L18 [UX]** Resume help does not point to where the previous run stopped. `/anvil resume` lists numbered steps with titles and ids (`src/index.ts:610-625`), but it ignores the prior run's last `step_start`, failure reason, and timestamp even though checkpoints contain that data. Marking the last started/failed step as the suggested default would make step selection safer after an abort or breaker trip.

- [ ] **L19 [DOCS]** Resume retry wording is ambiguous. The README and resume map say omitting `retry-number` means "no retries" (`README.md:43`, `src/index.ts:625`), but the engine still applies the workflow's normal `onFail.maxLoops`; omission only seeds `{loop}`/the current retry count as zero. Reword to "no retry count is seeded" or document whether resume is meant to disable further retries.

- [ ] **L20 [ROBUSTNESS]** Pi startup prompt detection can false-match subagent output. The missing-cwd detector treats any visible `cwd from session file does not exist` text as a blocked startup prompt (`src/subagent/exit.ts:90-93`), so a subagent that quotes the prompt while debugging or reviewing Anvil can be reported as failed before it writes its sidecar. Match the full selector shape (like the Continue/Cancel regex) or require prompt framing before returning the synthetic error.
